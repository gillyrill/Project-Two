...
//Second coin (Fin) for the lab assignment
function Fin(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.10));
  this.size = new Vector (0.6,0.6);
  this.wobble = Math.random() * Math.PI * 2;
}
Fin.prototype.type = 'fin';

//Portal for Project 2
function Portal(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0, 0));
  this.size = new Vector (0.6,3.6); 
  this.wobble = Math.random() * Math.PI * 2;
}
...
var wobbleSpeedP = 16, wobbleDistP = 0.14;
...
Fin.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin (this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector (0, wobblePos));
};
...
PowUp.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin (this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector (0, wobblePos));
};
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  if (obstacle)
    ...
    this.pos = newPos;
     // If player touches speed pow up, add to its playerXSpeed
};
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  if (obstacle == "portal") {
    motion = new Vector (this.pos.x,this.pos.y - 5);
    newPos = motion;
    this.pos = newPos;
  }
  //Github version doesn't use this, test it***
...
Player.prototype.act = function(step, level, keys) {
  ...
  Level.prototype.playerTouched = function(type, actor) {
    ...
    // Player loses
      ...
      this.status = "lost";
      this.finishDelay = 1;
      //Secondary  will be the win condition
    } else if (type == 'fin') {
  ...
  //Coins are collectible but not a win condition
      else if (type == 'coin') {
          this.actors = this.actors.filter(function(other) {
            return other != actor;
        });
